...
func getNewItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		err := dbx.Select(&items,
			"SELECT `items`.`id`, `items`.`seller_id`, `users`.`account_name`, `users`.`num_sell_items`, `items`.`buyer_id`, `items`.`status`, `items`.`name`, `items`.`price`, `items`.`description`, `items`.`image_name`, `items`.`category_id`, `items`.`created_at`, `items`.`updated_at` "+
				"FROM `items` "+
	...
	if itemID > 0 && createdAt > 0 {
		...
		inQuery, inArgs, err = sqlx.In(
			"SELECT `items`.`id`, `items`.`seller_id`, `users`.`account_name`, `users`.`num_sell_items`, `items`.`buyer_id`, `items`.`status`, `items`.`name`, `items`.`price`, `items`.`description`, `items`.`image_name`, `items`.`category_id`, `items`.`created_at`, `items`.`updated_at` "+
				"FROM `items` "+
				"JOIN `users` ON `items`.`seller_id` = `users`.`id` "+
				"WHERE `items`.`status` IN (?,?) AND `items`.`category_id` IN (?) "+
				"ORDER BY `items`.`created_at` DESC, `items`.`id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			categoryIDs,
